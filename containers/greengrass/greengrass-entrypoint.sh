...
#Default options
OPTIONS="-Droot=${GGC_ROOT_PATH} -Dlog.store=FILE -Dlog.level=${LOG_LEVEL} -jar ${INIT_JAR_PATH}/lib/Greengrass.jar --provision ${PROVISION} --deploy-dev-tools ${DEPLOY_DEV_TOOLS} --aws-region ${AWS_REGION} --start false --setup-system-service true"
...
echo "Looking for: Deployment for RosProvisioningGreengrassCore before starting greengrass..."

# Invoke the Py script to check the deployment
OUTPUT=$(python3 check_deployment.py)

# Loop until the deployment exists
while [ "$OUTPUT" != "True" ]; do
    echo "$OUTPUT"
    echo "Checking again in 20 seconds..."
    sleep 20
    OUTPUT=$(python3 check_deployment.py)
done
